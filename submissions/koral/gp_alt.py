"""
Phase α — Focused Electrostatic Global Placement (Stochastic Noise Version)
========================================================================

Alternative version of gp.py that adds Gaussian noise centered on the
current position after each Adam step.  The noise variance is equal to
the replica's temperature on the ladder.
"""
...
def _smooth_1d_along_axis(grid: torch.Tensor, smooth_range: int, axis: int) -> torch.Tensor:
    ...
    if axis == 0:
        ...
        cs0, cs1 = csum[..., 2 * sr:], torch.cat([torch.zeros(K, R, 1, device=grid.device, dtype=grid.dtype), csum[..., :C - 1 + 2 * sr]], dim=-1)
        return cs0[..., :C] - cs1[..., :C]
    else:
        R, C = grid.shape[-2:]
        ...
        cs0, cs1 = csum[..., 2 * sr:, :], torch.cat([torch.zeros(K, 1, C, device=grid.device, dtype=grid.dtype), csum[..., :R - 1 + 2 * sr, :]], dim=-2)
        return cs0[..., :R, :] - cs1[..., :R, :]
...
def focused_electrostatic_loss(
    ...
    threshold = sorted_d[:, k - 1:k].view(K, 1, 1)
    source = torch.clamp(density_grid - threshold, min=0.0)
...
def rudy_demand(
    pop: torch.Tensor, owner_idx: torch.Tensor, pin_off: torch.Tensor,
    net_id: torch.Tensor, n_nets: int, port_pos: torch.Tensor, n_macros: int,
    grid_col: int, grid_row: int, cw: float, ch: float,
...
def dfg_demand(
    ...
    net_id: torch.Tensor, n_nets: int, port_pos: torch.Tensor, n_macros: int,
    grid_col: int, grid_row: int, cw: float, ch: float,
    h_per_um: float, v_per_um: float, driver_idx_per_pin: torch.Tensor,
):
    K, device, dtype = pop.shape[0], pop.device, pop.dtype
    pins = _pin_positions(pop, owner_idx, pin_off, port_pos, n_macros)
    ...
    return v_demand / max(grid_w * v_per_um, 1e-9), h_demand / max(grid_h * h_per_um, 1e-9)
...
def run_global_placement(
    ...
    *,
    ...
    # Gaussian noise ladder: hottest replica covers canvas width with 10% prob.
    # P(|X| > W) = 0.10 => sigma = W / 1.645.  Variance = sigma^2.
    v_max = (cw / 1.645) ** 2
    if replica_temperatures is None:
        replica_temperatures = tuple(v_max * (0.01 ** (k / (pop_size - 1))) for k in range(pop_size))[::-1]
    ...
    temp_t = torch.tensor(replica_temperatures, device=device, dtype=torch.float32).view(-1, 1, 1)
    sigma_t = torch.sqrt(temp_t)
    ...
    for step in range(n_steps):
        ...
        opt.zero_grad()
        wl = smooth_hpwl(pop, owner_idx, pin_off, net_id, n_nets, port_pos, n_macros, gamma, cw, ch, n_nets_norm, weights_per_net=weights_per_net)
        ...
        with torch.no_grad():
            # Add Gaussian noise
            pop.data += torch.randn_like(pop) * sigma_t
        ...
        if verbose and step % log_every == 0: print(f"  [GP-Alt] step {step:4d} wl={wl.mean():.4f} dens={dens_loss.mean():.4f} cong={cong_loss.mean():.4f} ov={ov_loss.mean():.5f}", flush=True)
